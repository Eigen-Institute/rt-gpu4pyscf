...
def scan_r():
    for r in [0.74, 1.4]:
        ...
        ejk = float((_partial_ejk_ip2(mol, inter['P_I_prime'] + inter['P'], vhfopt) - _partial_ejk_ip2(mol, inter['P'], vhfopt))[0,0,2,2])
        ejk += 0.5 * float(_partial_ejk_ip2(mol, inter['R_I'] + inter['R_I'].T, vhfopt)[0,0,2,2])
        ...
        static = hcore + ejk + ovlp
        print(f"\nR={r}:")
        print(f"  Static:    {static:.6f}")
        print(f"  Perturbed: {pert:.6f}")
        print(f"  Sum:       {static + pert:.6f}")
