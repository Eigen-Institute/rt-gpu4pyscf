...
def diagnose_manual():
    ...
    h_hcore = float(de_hcore(0,0)[0,0])
    ...
    for j in range(2):
        if j == 0: continue
        q0, q1 = aoslices[j][2:]
        val_ovlp -= float(contract('pq,pq->', W[p0:p1, q0:q1], s1aa[0,0,p0:p1, q0:q1])) * 2.0
    ...
    val_eri = float(h_eri_raw[0,0,0,0]) * 0.5
    ...
    s1ao_x = cp.zeros((2, 3, nao, nao))
    for atm_id in range(2):
        p0c, p1c = aoslices[atm_id][2:]; s1ao_x[atm_id, :, p0c:p1c] += s1a_basis[:, p0c:p1c]; s1ao_x[atm_id, :, :, p0c:p1c] += s1a_basis[:, p0c:p1c].transpose(0, 2, 1)
    s1mo_x = cp.zeros((2, 3, nao, nao))
    for i in range(2):
        for j in range(3): s1mo_x[i, j] = cp.asarray(mo_coeff.T) @ s1ao_x[i, j] @ cp.asarray(mo_coeff)
    Ux = cp.zeros((2, 3, nao, nao)); Ux[:, :, :, :nocc] = mo1
    Ux[:, :, :nocc, nocc:] = (-s1mo_x[:, :, :nocc, nocc:] - mo1[:, :, nocc:, :].transpose(0, 1, 3, 2))
    Ux[:, :, nocc:, nocc:] = -0.5 * s1mo_x[:, :, nocc:, nocc:]
    
    x1, y1 = solve_cptddft(h_obj, x_y, td.e[0], mo1, mo_e1)
    pert_inter = make_perturbed_intermediates(h_obj, inter, x_y, x1, y1, Ux, z1)
    
    h1 = cp.asarray(-mol.intor('int1e_ipkin', comp=3) - mol.intor('int1e_ipnuc', comp=3)); h1ao_x = cp.zeros((2, 3, nao, nao))
    for atm_id in range(2):
        ...
        h1ao_x[atm_id] = vrinv * 2.0; h1ao_x[atm_id, :, p0c:p1c] += h1[:, p0c:p1c]; h1ao_x[atm_id, :, :, p0c:p1c] += h1[:, p0c:p1c].transpose(0, 2, 1)

    ip1_hcore = float(cp.trace(h1ao_x[0,0] @ pert_inter['P_I_prime_y'][0,0]))
    ip1_ovlp = -float(cp.trace(s1ao_x[0,0] @ pert_inter['W_I_y'][0,0])) * 2.0
    ...
    ip1_eri = float(cp.trace(G_PI_x[0,0] @ pert_inter['P_y'][0,0]) + cp.trace(G_P_x[0,0] @ pert_inter['P_I_prime_y'][0,0]) + cp.trace(G_PI_x[0,0] @ pert_inter['P_I_prime_y'][0,0]))
    ip1_eri += float(cp.trace(G_RI_x[0,0] @ (pert_inter['R_I_y_ao'][0,0] + pert_inter['R_I_y_ao'][0,0].T)))
    ...
    c_p = coords.copy(); c_p[0,0] += dr
    g_p = get_g(c_p)
    c_m = coords.copy(); c_m[0,0] -= dr
    g_m = get_g(c_m)
    h_fd = (g_p[0,0] - g_m[0,0]) / (2 * dr)
    ...
    print(f"Total Analytical X-X [0,0]: {total_anal:.8f}")
    print(f"Semi-Analytical X-X:         {h_fd:.8f}")
